...
def build_manual(template, project, captures, destination):
    ...
    doc.add_paragraph("在Windows 10或11安装Python 3.11及以上版本，双击启动软件.cmd。首次启动会创建虚拟环境并安装PyQt6。后续启动复用同一环境。")
    doc.add_paragraph("内置演示账号为admin，密码为Admin123!。注册的新账号为操作员，最终通过或退回操作由管理员执行。样例仅用于演示和测试。")
    doc.add_paragraph("输入区使用JSON格式，保留字段名、引号和逗号。无效输入会显示说明；业务数据和复核记录保存在data/app.sqlite。")
    ...
    doc.add_paragraph("输入格式错误时，请检查JSON语法及字段类型；业务规则错误时，请根据提示调整范围。复核前必须完成计算，已通过记录不能直接改回草稿，需要重新计算建立新记录。")
